...
export function ActivitiePreview({ act, getRowIdx, onRemoveAct, onEdit }) {
    ...
    return (
        <div ref={drag} className={`activity-prev-assembly activity-assembly ${isDayHeadClass} ${isFirstCol}`} style={{ gridRow: `${act.row || 'auto'}/span ${act.duration}`, opacity: opc }} key={utils.makeId()}>
            <h2>{act.name}</h2>
